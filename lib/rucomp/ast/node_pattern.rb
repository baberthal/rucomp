...
module Rucomp
  module Ast
    ...
    #
    # Initialize a new `NodePattern` with `NodePattern.new(pattern_string)`,
    # then pass an AST node to `NodePattern#match`. Alternatively, use one of
    # the class macros in `NodePattern::Macros` to define your own
    ...
    #                        # in other words, while {} is pattern union
    #                        # (logical OR), [] is intersection (logical AND)
    #    '(send %1 _)'       # % stands for a parameter which must be supplied
    #                        # to #match at matching time
    ...
    #    '#method'           # we call this a 'funcall'; it calls a method in
    #                        # the context where a pattern-matching method is
    #                        # defined if that returns a truthy value, the match
    #    'equal?(%1)'        # succeeds predicates can be given 1 or more extra
    #                        # args
    ...
    class NodePattern
      ...
      class Compiler
        ...
        META    = /\(|\)|\{|\}|\[|\]|\$\.\.\.|\$|!|\^|\.\.\./
        NUMBER  = /-?\d+(?:\.\d+)?/
        TOKEN   = /\G(?:[\s,]+|#{META}|\#?#{ID_CHAR}+[\!\?]?\(?|%\d*|#{NUMBER}|#{RSYM}|.)/
